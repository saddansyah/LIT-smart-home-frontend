    ...
    <RouterLink :to="{ name: 'Device Details', params: { deviceId: device.id } }">
        <div
            ...
            <div class="device-container flex flex-row w-full h-full gap-6">
                <div v-ripple class="device-title flex flex-col basis-[20%] min-w-fit p-6 bg-slate-50 rounded-xl shadow justify-center">
                ...
                <div v-ripple class="inline-block w-full h-full min-w-fit basis-[60%] p-6 bg-slate-50 rounded-xl shadow justify-center">
                    <div v-if="!totalPower[0]">
                        <GraphLoading />
                    </div>
                    <div v-else>
                        <PeakPowerChart :chartId="chartId" :data="devicePowers"/>
                    </div>
                </div>
            </div>
        </div>
...
<script setup>
import { PeakPowerChart, GraphLoading } from "@/utils/componentLoader.js";
